Shader "GGJ/Vertex Color Ground" {
...
VertexToFragment VertexProgram (VertexInput vertex)
{
    ...
    output.worldPosition = mul(_Object2World, vertex.position)+ half4(0,2*sin(0.75*_Time.y+ vertex.position.x + vertex.position.z + vertex.position.y),0,0);
    ...
    output.lightColor = NDotL*_DLightColor.rgb + NDotLMoon*_MoonLightColor.rgb + _Ambient.rgb;
    //output.color = vertex.color.rgb * lightColor*(1-fog.a) + fog.rgb * fog.a;
    return output;
}
